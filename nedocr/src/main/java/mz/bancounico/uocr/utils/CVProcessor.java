...
public class CVProcessor {
    ...
    public static List<MatOfPoint> findContours(Mat src){
        ...
        Imgproc.GaussianBlur(resizedImg,resizedImg,new Size(7,7),0);
       // Imgproc.medianBlur(resizedImg, resizedImg, 7);

        Mat cannedImg = new Mat(newSize, CvType.CV_8UC1);
        Imgproc.Canny(resizedImg, cannedImg, 20, 120, 3, true);
//        Imgproc.Canny(resizedImg, cannedImg, 30, 120, 3, true);
        resizedImg.release();

       // Imgproc.threshold(cannedImg, cannedImg, 70, 255, Imgproc.THRESH_OTSU);
        //Imgproc.medianBlur(src, src, 1);
        //Imgproc.adaptiveThreshold(cannedImg,cannedImg,195,Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C,Imgproc.THRESH_BINARY,31,2);

//        Imgproc.GaussianBlur(src, src, new Size(7, 7), 0);
        //Imgproc.threshold(cannedImg, cannedImg, 0, 255, Imgproc.THRESH_BINARY+ Imgproc.THRESH_OTSU);
       ...
       // Imgproc.findContours(dilatedImg, contours, hierarchy, Imgproc.RETR_LIST, Imgproc.CHAIN_APPROX_SIMPLE);
        hierarchy.release();
        dilatedImg.release();

        Log.d(TAG, "contours found: " + contours.size());

        Collections.sort(contours, (o1, o2) -> Double.valueOf(Imgproc.contourArea(o2)).compareTo(Imgproc.contourArea(o1)));
